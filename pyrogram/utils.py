...
async def get_input_stargift(client: "pyrogram.Client", owned_gift_id: str) -> "raw.base.InputSavedStarGift":
    ...
    if saved_gift_match:
        ...
        try:
            ...
            raise ValueError(f"Invalid owned_gift_id format: {owned_gift_id!r}")
    ...
    if isinstance(
        ...
        if replies:
            ...
            if messages_with_replies:
                ...
                chat_id = next((m.chat.id for m in parsed_messages if m.chat), None)
                if chat_id is None:
                    log.warning("Cannot fetch replies: no chat found in parsed messages")
...
def parse_deleted_messages(client, update, users, chats) -> List["types.Message"]:
    ...
    if peer:
        ...
        if chat_id:
            if isinstance(peer, raw.types.PeerUser) and chat_id in users:
                chat = types.Chat._parse_user_chat(client, users[chat_id])

            elif isinstance(peer, raw.types.PeerChat) and chat_id in chats:
...
def unpack_inline_message_id(inline_message_id: str) -> "raw.base.InputBotInlineMessageID":
    ...
    if len(decoded) == 20:
        ...
        raise ValueError(f"Invalid inline message id: expected 20 or 24 bytes, got {len(decoded)}")
...
def get_raw_peer_id(peer: Union[int, raw.base.Peer, raw.base.InputPeer, raw.base.RequestedPeer]) -> Optional[int]:
    ...
    if isinstance(peer, int):
        if peer < 0:
            ...
            if ZERO_CHANNEL_ID - MAX_MONOFORUM_CHANNEL_ID <= peer and peer != ZERO_CHANNEL_ID:
                return ZERO_CHANNEL_ID - peer

            if ZERO_SECRET_CHAT_ID + (-1 << 31) <= peer and peer != ZERO_SECRET_CHAT_ID:
                return ZERO_SECRET_CHAT_ID - peer
...
def expand_inline_bytes(bytes_data: bytes):
    ...
    header = bytearray(_JPEG_HEADER)

    header[164] = bytes_data[1]
    header[166] = bytes_data[2]

    return header + bytes_data[3:] + _JPEG_FOOTER
