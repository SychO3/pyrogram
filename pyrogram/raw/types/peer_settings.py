...
class PeerSettings(TLObject):  # type: ignore
    ...
    Parameters:
        ...
        business_bot_paused (optional): ``bool``
        business_bot_can_reply (optional): ``bool``
        geo_distance (optional): ``int`` ``32-bit``
        request_chat_title (optional): ``str``
        request_chat_date (optional): ``int`` ``32-bit``
        business_bot_id (optional): ``int`` ``64-bit``
        business_bot_manage_url (optional): ``str``
        charge_paid_message_stars (optional): ``int`` ``64-bit``
        registration_month (optional): ``str``
        phone_country (optional): ``str``
        name_change_date (optional): ``int`` ``32-bit``
        photo_change_date (optional): ``int`` ``32-bit``
    """
