...
class UrlAuthResultRequest(TLObject):
    ...
    Parameters:
        ...
        request_phone_number (optional): ``bool``
        match_codes_first (optional): ``bool``
        is_app (optional): ``bool``
        browser (optional): ``str``
        platform (optional): ``str``
        ip (optional): ``str``
        region (optional): ``str``
        match_codes (optional): List of ``str``
        user_id_hint (optional): ``int`` ``64-bit``
        verified_app_name (optional): ``str``
